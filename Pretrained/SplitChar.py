...
def doFile(inputFile, outputFile) :
    ...
    with PIL.Image.open(inputFile) as image :
        ...
        for i in range(len(digits)) :
            ...
            digitImage = tf.keras.preprocessing.image.array_to_img(digits[i])
            digitImage = PIL.ImageOps.pad(digitImage, (20,20), color='black')
            digitImage = PIL.ImageOps.expand(digitImage, border=4, fill='black')
...
def convertMonochrome(image) :
    ...
    image = PIL.ImageEnhance.Brightness(image).enhance(1.7)
    showPILOnPlot(3, image, 'brightness')

    image = PIL.ImageEnhance.Contrast(image).enhance(20)
...
def splitDigits(tensor) :
    ...
    while column < len(column_totals) - 1 :    
        print('looking at', column)
        digit_start = searchForNonZero(column_totals, column, 1, 1)
        digit_end = searchForZero(column_totals, digit_start, 1, 5)

        if digit_start == digit_end :
            print('did not find digit at', column)
            break

        print('found digit at', digit_start, digit_end)
        digits.append(cropHeight(tensor[ : , digit_start:digit_end]))
...
def cropHeight(tensor) :
    ...
    bottom = searchForNonZero(row_totals, len(row_totals) - 1, -1, blur=2)
...
def cropWidth(tensor) :
    ...
    right = searchForNonZero(column_totals, len(column_totals) - 1, -1, blur=2)
...
def saveDigit(outputFile, index, image) :
    op = pathlib.Path(outputFile)
    digitName = op.stem + '_' + str(index) + op.suffix
