...
class Insect(Organism) :
    ...
    def draw(self, position) :
        if enableDraw :
            v = self.direction if self.direction.length() > 0.1 else pygame.Vector2([1,0])
            ...
            pygame.draw.polygon(getField(), self.color, [p1,p2,p3])
    ...
    def moveTo(self, newPosition) :
        ...
        organism = anyCollision(newPosition, ignore = self, ignoreType = Insect)
    ...
    def tick(self, secondsSinceLastFrame) :
        ...
        if self.direction.length() > 0.1 :
            self.direction.scale_to_length(self.speed * secondsSinceLastFrame)
...
class Chromosome :
    ...
    def mate(self, other) :
        ...
        for i in range(len(self.gene)) :
            child.gene[i] = random.choice([self.gene[i], other.gene[i]])
        return child
...
class Bee(Insect) :
    ...
    def isHome(self) :
        return distanceFrom(self.position, self.nestLocation) < 10
    ...
    def moveDirection(self) :
            ...
            if self.isHome() :
                if self.atHomeCounter > 0 :
                    ...
                    if self.carryingFood :
                        ...
                        self.targetLocation = self.nest.getFoodLocation()
                        self.foodLocation = None
        else :
        ...
        if random.randint(0,100) < self.nest.chromosome.fidgetChance() :
            ...
            if self.targetLocation != None and random.randint(0,100) < self.nest.chromosome.concentrateTarget() :
                return vectorTo(self.targetLocation, self.position)
            else :
...
if __name__ == '__main__' :
    ...
    insectMainLoop(boardSize = (1000,700))
