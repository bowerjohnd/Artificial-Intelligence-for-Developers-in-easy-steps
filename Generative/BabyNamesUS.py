...
# US National data format: Name,F,count (Mary,F,8012)

# around line 157, choose file year, currently using (1883,M)
def readNames(year, genders='FM', nameSize=99) :
    names = set()

    with open('source/yob' + str(year) + '.txt', newline='') as csvfile :
        nameReader = csv.reader(csvfile, delimiter=',', quotechar='"')

        # Modified from Scotland format to US National format
...
def lstmProject() :
    names = readNames(year='1883', genders='M') # ssa.gov national baby names by year zip file
    dataset = Names(names)
