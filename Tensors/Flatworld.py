...
def randomDrop() :
    ...
    while True :
        position = pygame.Vector2(random.random() * w, random.random() * h)
        
        organism = anyCollision(position)
        if organism == None :
            return position

def anyCollision(position, ignore = None, ignoreType = type(None)) :

    for organism in organisms :
        if organism != ignore and not isinstance(organism, ignoreType) :
...
# if you want Red to pass through grass, comment out the 'return organism' above and uncomment the following:
...
class Grass(Organism) :
    ...
    def draw(self, position) :
        ...
        s = 20 // self.leaves if self.leaves > 0 else 20
...
def mainLoop(initialSpawns, spawnInterval, periodicSpawns, boardSize, playerType) :
    ...
    while any([isinstance(organism, playerType) for organism in organisms]) and running :
        tickCounter += 1
