...
NUM_EXTRAS = 6
NUM_OUTPUTS = 1
NUM_PERCEPTRONS = NUM_DIRECTIONS * NUM_VARIANTS + NUM_EXTRAS + NUM_OUTPUTS
...
def getSpots(me) :
    ...
    redList = filter(lambda x: type(x) == Carnivore and x != me, organisms)
    blueList = filter(lambda x: type(x) == Herbivore and x != me, organisms)
    grassList = filter(lambda x: type(x) == Grass, organisms)

    redSpots = list(map(lambda x: Spot(me, x), redList))
    blueSpots = list(map(lambda x: Spot(me, x), blueList))
...
def getZones(spots) :
    ...
    zonesTotals = reduce(lambda a,b: addTotals(a,b), zonesList)
...
class Chromosome :
    def __init__(self, chromosome = None) :
        if chromosome == None :
            ...
            for y in range(NUM_PERCEPTRONS) :
                matrix.append([random.random() * 2 - 1 for x in range(NUM_PERCEPTRONS)])
    ...
    def mutate(self) :
        ...
        for i in range(NUM_MUTATIONS) :
            ...
            mine[y][x] *= (random.random() * 1.5 + 0.5) * (random.randint(0, 1) * 2 - 1)
    ...
    def __str__(self) :
        ...
        for op in range(NUM_PERCEPTRONS) :
            ...
            for ip in range(NUM_PERCEPTRONS) :
                s = s + caption(ip) + '{0:+8.3g}'.format(self.gene[ip][op]) + ' '
...
class Creature(Insect) :
    def __init__(self, chromosome, position, color) :
        ...
        self.extraPerceptrons = [random.gauss(0, 1) for i in range(NUM_EXTRAS)]

        self.direction = pygame.Vector2((-1, 0))
        self.direction = self.direction.rotate(45 * random.randint(0, 9))
    ...
    def moveDirection(self) :
        ...
        result = runNetwork(self.perceptrons, self.chromosome.gene)

        outputs = np.array(result[:NUM_OUTPUTS])
        self.extraPerceptrons = list(np.array(result[NUM_OUTPUTS : NUM_OUTPUTS + NUM_EXTRAS]))
...
class Carnivore(Creature) :
    def __init__(self, chromosome, position) :
        super().__init__(chromosome, position, pygame.Color('red'))
...
class Herbivore(Creature) :
    def __init__(self, chromosome, position) :
        super().__init__(chromosome, position, pygame.Color('blue'))
...
def countCreatures(creatures, kind) :
    return len(list(filter(lambda a: type(a) == kind, creatures)))
...
def oneGeneration(clock,
    ...
    frameDecimation = 10                   # default 100, lower may include flashing (photosensitivity), higher is safer
    redStartPopulation = len(redChromosomes)
    blueStartPopulation = len(blueChromosomes)

    for chromosome in redChromosomes :
        organisms.append( Carnivore(chromosome, randomDrop()) )
    
    for chromosome in blueChromosomes :
        organisms.append( Herbivore(chromosome, randomDrop()) )
...
def neuralMainLoop(boardSize, population, generations) :
    ...
    redChromosomes = [Chromosome() for i in range(numReds)]
    blueChromosomes = [Chromosome() for i in range(numBlues)]
    ...
    for generation in range(1, generations + 1) :
        ...
        # usually only lasts to 299 with default limitations:
        
        repeats = 2000   # default 200
    ...
    creatures, frameCount = oneGeneration(clock,
                                          redChromosomes,
                                          blueChromosomes,
                                          repeats,
                                          generation)
    ...
    reds = list(filter(lambda a: type(a) == Carnivore, creatures))
    blues = list(filter(lambda a: type(a) == Herbivore, creatures))

    if len(reds) > 0 :
        reds = sorted(reds, key = attrgetter('score'), reverse = True)
        redChromosomes = nextGeneration(reds, numReds)
        redScore = reduce(lambda a, b: a + b.score, reds, 0) / len(reds)

        print('Red chromsoome:\n', str(reds[0].chromosome))
        print('Red score average:', redScore)

    if len(blues) > 0 :
        blues = sorted(blues, key = attrgetter('score'), reverse = True)
        blueChromosomes = nextGeneration(blues, numBlues)
        blueScore = reduce(lambda a, b: a + b.score, blues, 0) / len(blues)
