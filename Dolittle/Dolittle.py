...
class Query :
    ...
    def ask(self, possible) :
        answer = tk.messagebox.askquestion(title = 'Question', message = self.question)
...
class myCheckbutton(tk.Checkbutton) :
    def __init__(self, top, variable, text, row, column, option) :
        self.var = tk.IntVar()
        self.variable = variable
        self.option = option
        super().__init__(top, text=text, variable=self.var, onvalue=1, offvalue=0, command=self.callback)
...
class CompleteQueryDialog :
    def __init__(self, parent, question, options) :
        ...
        tk.Label(self.frame, text=text).grid(row=0, column=0, columnspan=2, padx=10)
        self.okButton = tk.Button(self.frame, text='OK', command=self.onOK)
        self.okButton.grid(row=0, column=2)

        tk.Label(self.frame, text=question).grid(row=1, column=0, columnspan=2)
        tk.Label(self.frame, text='Yes').grid(row=2, column=1)

        index = 0
        widgets = []
        for option in options :
            tk.Label(self.frame, text=option).grid(row=index+3, column=0)
            widgets.append(myCheckbutton(self.frame, self.yeses, '', index+3, 1, option))
            index = index + 1
